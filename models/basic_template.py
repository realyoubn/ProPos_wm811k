...
class TrainTask(object):
    ...
    def build_default_options():
        ...
        parser.add_argument('--use_weighted_sampler', action='store_true', help='是否使用加权采样器处理不平衡数据')
        return parser
    ...
    def build_dataloader(self,
        ...
        # # 处理数据不平衡问题，特别是wm811k数据集
        # weighted_sampler = None
        # if train and dataset_name == 'wm811k' and opt.use_weighted_sampler:
        #     # 计算每个类的权重
    ...
    def train_transform(self, normalize):
        ...
        # 对于彩色图像应用颜色抖动，对于灰度图像跳过
        if opt.dataset != 'wm811k' and opt.dataset != 'mnist':
            train_transform.append(transforms.RandomApply([
                transforms.ColorJitter(0.4, 0.4, 0.4, 0.1)
            ], p=0.8))
    
        # 灰度图像不需要再次应用灰度变换
        if opt.dataset != 'wm811k' and opt.dataset != 'mnist':
            train_transform.append(transforms.RandomGrayscale(p=0.2))
    ...
    def normalize(dataset_name):
        normalize_params = {
            ...
            'wm811k': [(0.5,0.5,0.5), (0.5,0.5,0.5)]  # 添加wm811k灰度图像标准化参数
        }
