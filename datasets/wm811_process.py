...
import time
import argparse
from typing import Union  # 新增：用于正确的类型注解
...
class WM811kProcessor(object):
    def __init__(self, wm811k_file: str):
        ...
        self.data['trainTestLabel'] = self.data['trianTestLabel'].apply(self.getTrainTestLabel)  # 这里修正了拼写
    ...
    # 修正：类型注解用Union[list, tuple]替代list or tuple
    def write_images(self, root: str, indices: Union[list, tuple]):
        """Write wafer images to .png files."""
        os.makedirs(root, exist_ok=True)
        # 修正：直接配置tqdm参数，移除get_tqdm_config依赖
        with tqdm.tqdm(total=len(indices), leave=True, colour='yellow') as pbar:  # 注意颜色参数是colour（tqdm标准参数）
